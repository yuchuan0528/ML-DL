...
### 6.6 为什么不存Q?
* Q 是瞬态的 (Transient)： q_jumps 在完成了查询缓存和 k_jumps 的任务后，就没有用了。

* K/V 是上下文 (Context)： k_jumps 和 v_jumps 代表了 "jumps" 这个 token 作为历史上下文的“标签”和“信息”，它们必须被储存起来，供未来的 token (如 "over", "the", "lazy", "dog"...) 来查询。

一句话总结：KV Cache 储存的是历史 K/V 对，而 Q 是当前 token 用来查询这些历史的“钥匙”，Q 本身不是历史的一部分，所以不用缓存。
